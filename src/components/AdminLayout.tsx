...
import { LogOut, Calendar, Users, BarChart, Layout, UserPlus, Moon, Sun } from 'lucide-react';
         ...
         <div className="max-w-[1920px] mx-auto px-6 sm:px-12 h-20 flex items-center justify-between">
             ...
             <Link to="/" className="flex items-center gap-4 text-white font-serif text-2xl tracking-tighter group active:scale-95 transition-transform">
               ...
               {currentUser && (
                 <div className="ml-4 pl-4 border-l border-white/10">
                   <Avatar name={currentUser.username} avatarUrl={currentUser.avatar_url} size="sm" />
                 </div>
               )}
             </Link>
               ...
               {navItems.map(item => {
                 ...
                 return (
                   <Link 
                     key={item.path} 
                     to={item.path}
                     className={`relative flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all group ${isActive ? 'text-white' : 'text-white/30 hover:text-white/60 hover:bg-white/[0.02]'}`}
                   >
                     {isActive && (
                       <motion.div
           ...
           <div className="flex items-center gap-6">
             ...
             <button 
               ...
               className="group flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-white/5 border border-white/5 hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-400 transition-all active:scale-95 group"
             >
             ...
             <button 
               onClick={toggleTheme}
               className="group flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 transition-all active:scale-95"
